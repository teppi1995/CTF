...
class HalfFeed(object):
    ...
    def encrypt(self, nonce, plaintext):
        ...
        logging.info("self.key and nonce encryption")
        logging.info("nonce is : {}".format(nonce))
        logging.info("make Kn and _")
        
        T, K = aes_encrypt(Kn, nonce)
        logging.info("Kn and nonce encryption")
        logging.info("nonce is : {}".format(nonce))
        logging.info("make T and K")
        
        ciphertext = b''
        for i in range(0, len(plaintext), 16):
            logging.info("tag is : {}".format(T.hex()))
